...
# 홈 포지션 (카메라 시작 시 초기화 자세)
# J1_HOME: 베이스 회전 각도 — 실측 후 정면을 바라보는 값으로 조정하세요
HOME_J1 = 90   # ← 실제 정면 각도로 수정 필요
HOME_J2 = 120  # HOVER_J2와 동일 (들어올린 안전 높이)
HOME_J3 = 90   # HOVER_J3와 동일
...
DISCARD_RIGHT_J1 = 180  # 불량품 → 오른쪽 폐기함 방향
...
def reset_to_home():
    """
    카메라 시작 시 로봇팔을 홈 포지션으로 초기화합니다.
    - 그리퍼 열기
    - 베이스(J1)를 정면 방향(HOME_J1)으로 회전
    - 팔을 안전한 호버 높이(HOME_J2, HOME_J3)로 들어올리기

    아두이노가 연결되지 않은 시뮬레이션 모드에서는 메시지만 출력합니다.
    """
    if ser is None:
    ...
    try:
        # 2000ms(2초)에 걸쳐 천천히 홈으로 복귀 (급격한 움직임 방지)
        send_robot_command("M", GRIPPER_OPEN, HOME_J1, HOME_J2, HOME_J3, move_time=2000)
        time.sleep(2.5)  # 관절 이동 완료까지 대기 (cap 루프 전이라 keep_alive_sleep 불필요)
...
ELLIPSE_RATIO_THRESHOLD  = 0.94  # 타원 단축/장축 비율; 이 값 미만이면 찌그러짐으로 판정
                                  # 1.0 = 완전한 원, 낮을수록 납작하게 찌그러진 공

BRIGHTNESS_STD_THRESHOLD = 60.0  # 공 내부 밝기 표준편차; 높으면 불규칙한 반사 → 불량
                                  # 현재는 비활성화 상태. 조명 환경 실측 후 활성화 권장

DENT_THRESHOLD = 50  # 공 내부 Canny 엣지 픽셀 수; 많을수록 표면 흠집 가능성 높음 (보조 판단)

COOLDOWN_SEC = 7.0    # 한 번 공을 처리한 후 다음 감지까지 기다리는 시간(초)
                      # 로봇 동작 중 중복 트리거를 방지
...
# ── 홈 포지션 초기화 ──────────────────────────────────────────
# 카메라가 정상적으로 열린 직후, 메인 루프 시작 전에 로봇팔을 홈으로 복귀
reset_to_home()
